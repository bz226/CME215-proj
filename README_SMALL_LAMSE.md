...
The activation helper sets `JAX_PLATFORMS=cuda`, `JAX_ENABLE_X64=True`, and conservative thread limits. By default, JAX is installed with pip-bundled CUDA/cuDNN (`jax[cuda12]`) to avoid local CUDA/cuDNN mismatches across Sherlock GPU nodes. Run JAX smoke tests from a GPU allocation; CPU-only device discovery can fail on constrained Sherlock allocations.
