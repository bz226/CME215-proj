...
sbatch \
  ...
  --cpus-per-task=8 \
  --mem=256G \
  --export=ALL,PROJECT_DIR="${PROJECT_DIR}",ANALYSIS_PATH="${ANALYSIS_PATH}",PARAMS_DIR="${PARAMS_DIR}",CHECKPOINT="${CHECKPOINT}",BATCH_NUMBER="${RUN_BATCHES}",CHECKPOINT_EVERY="${CHECKPOINT_EVERY}",OPT_CHECKPOINT_EVERY="${OPT_CHECKPOINT_EVERY}",CSV_PATH="${CSV_PATH}",SEED="${SEED}" \
