...
CHECKPOINT="${CHECKPOINT:-params/graphcast_small_amse.${START_TAG}.npz}"
...
sbatch \
  ...
  --cpus-per-task=8 \
  --mem=256G \
  --export=ALL,PROJECT_DIR="${PROJECT_DIR}",ANALYSIS_PATH="${ANALYSIS_PATH}",CHECKPOINT="${CHECKPOINT}",BATCH_NUMBER="${RUN_BATCHES}",CHECKPOINT_EVERY="${CHECKPOINT_EVERY}",OPT_CHECKPOINT_EVERY="${OPT_CHECKPOINT_EVERY}",CSV_PATH="${CSV_PATH}",SEED="${SEED}" \
