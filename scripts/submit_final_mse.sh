...
# Submit the final weighted-MSE control run on Sherlock.
#
# Default final target:
#   total MSE batches: 5000
...
#
# Resume after an existing MSE checkpoint:
#   START_BATCH=1000 bash scripts/submit_final_mse.sh
...
CHECKPOINT="${CHECKPOINT:-${PARAMS_DIR}/graphcast_small_mse.${START_TAG}.npz}"
CSV_PATH="${CSV_PATH:-runs/mse_final_${START_TAG}_to_${FINAL_TAG}.csv}"

cat <<EOF
Submitting final MSE control:
...
sbatch \
  ...
  scripts/sbatch_mse_training.sh
