#!/usr/bin/env bash
# Pure AMSE Sherlock training job. This wrapper always runs --spectral-amse and
# never routes through --lamse or LAMSE_LAMBDA.
#
...
if [[ -z "${PROJECT_DIR:-}" ]]; then
  if [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "${SLURM_SUBMIT_DIR}/scripts/run_amse_training.sh" ]]; then
...
if [[ ! -f "${PROJECT_DIR}/scripts/run_amse_training.sh" ]]; then
  cat >&2 <<EOF
...
Or pass PROJECT_DIR explicitly:
  PROJECT_DIR=/path/to/graphcast-small-lamse sbatch scripts/sbatch_amse_training.sh
EOF
  exit 2
fi
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${SLURM_JOB_NAME:-gc-amse-train}-${SLURM_JOB_ID:-manual}.log}"
...
Resubmit with an explicit Sherlock GPU constraint, for example:
  sbatch -C GPU_MEM:80GB scripts/sbatch_amse_training.sh
EOF
    exit 2
  fi
else
