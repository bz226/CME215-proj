#!/usr/bin/env bash
# Default weighted-MSE Sherlock training job. This wrapper does not pass
# --spectral-amse or --lamse; train.py therefore uses its built-in spatial MSE.
#
...
if [[ -z "${PROJECT_DIR:-}" ]]; then
  if [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "${SLURM_SUBMIT_DIR}/scripts/run_mse_training.sh" ]]; then
...
if [[ ! -f "${PROJECT_DIR}/scripts/run_mse_training.sh" ]]; then
  cat >&2 <<EOF
...
Or pass PROJECT_DIR explicitly:
  PROJECT_DIR=/path/to/graphcast-small-lamse sbatch scripts/sbatch_mse_training.sh
EOF
  exit 2
fi
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${SLURM_JOB_NAME:-gc-mse-train}-${SLURM_JOB_ID:-manual}.log}"
...
CSV_PATH="${CSV_PATH:-${OUTPUT_DIR}/mse_full_job_${SLURM_JOB_ID:-manual}.csv}"
...
Resubmit with an explicit Sherlock GPU constraint, for example:
  sbatch -C GPU_MEM:80GB scripts/sbatch_mse_training.sh
EOF
    exit 2
  fi
else
